 ...
 * @param grad_out_distloss Tensor of shape (H, W) containing the gradient of the output distance loss
 ...
 * 	- Tensor of shape (N, 1) containing the gradient of the densities
...
rasterizeOctreeVoxelsBackwardCUDA(
	const torch::Tensor& background,
	const torch::Tensor& positions,
	const torch::Tensor& colors,
	const torch::Tensor& densities,
