...
{
	return x < 10.0f ? log1pf(expf(x)) : x;
}


__forceinline__ __device__ float softplus_prime(float sx)
{
	return -expm1f(-sx);
}
...
__forceinline__ __device__ float sigmoid_prime(float s)
{
	return s * (1.0f - s);
}
...
template <typename T>
__forceinline__ __device__ T lerp(T v0, T v1, T t) {
...
__forceinline__ __device__ float3 getRayDir(
	...
	float x = (2.0f * pix.x / width - 1.0f) * tan_fovx;
	float y = (2.0f * pix.y / height - 1.0f) * tan_fovy;
	float3 ray_dir = {
