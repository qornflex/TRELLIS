...
namespace OctreeTrivecRasterizer {
 ...
 * @param trivecs 			Tensor of shape (N, 8, 3, 8) containing the trivectors of the octree nodes
 ...
 * @param colors_overwrite 	Tensor of shape (N, 3) containing the colors to overwrite the trivector colors
 * 
 ...
 * 
 * @return A tuple containing:
 * 	- Tensor of shape (N, 8, 3, 8) containing the gradient of the trivectors
...
rasterizeOctreeTrivecsBackwardCUDA(
	const torch::Tensor& background,
	const torch::Tensor& positions,
	const torch::Tensor& trivecs,
	const torch::Tensor& densities,
