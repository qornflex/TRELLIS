...
OctreeTrivecRasterizer::rasterizeOctreeTrivecsCUDA(
	const torch::Tensor& background,
	const torch::Tensor& positions,
	const torch::Tensor& trivecs,
	...
	const torch::Tensor& colors_overwrite
) {
	// Sizes
	...
	torch::Tensor out_percent_depth = torch::zeros({H, W}, float_opts);
	...
	if (P > 0) {
		...
		int trivec_rank = trivecs.size(1);
		int trivec_dim = trivecs.size(3);
		rendered = OctreeTrivecRasterizer::CUDA::forward(
			...
			trivecs.contiguous().data_ptr<float>(),
			trivec_rank, trivec_dim,
			...
			colors_overwrite.contiguous().data_ptr<float>(),
			out_color.contiguous().data_ptr<float>(),
			out_depth.contiguous().data_ptr<float>(),
			out_alpha.contiguous().data_ptr<float>(),
			out_percent_depth.contiguous().data_ptr<float>()
	...
	return std::make_tuple(
		rendered, out_color, out_depth, out_alpha, out_percent_depth,
...
OctreeTrivecRasterizer::rasterizeOctreeTrivecsBackwardCUDA(
	const torch::Tensor& background,
	const torch::Tensor& positions,
	const torch::Tensor& trivecs,
	const torch::Tensor& densities,
	...
	// Allocate output tensors
	torch::Tensor grad_trivecs = torch::zeros_like(trivecs);
	torch::Tensor grad_densities = torch::zeros_like(densities);
	torch::Tensor grad_shs = shs.size(0) == 0 ? torch::empty({0}, positions.options()) : torch::zeros_like(shs);
	torch::Tensor grad_colors = torch::zeros({P, trivecs.size(1), CHANNELS}, positions.options());
	...
	if (P > 0) {
		...
		int trivec_rank = trivecs.size(1);
		int trivec_dim = trivecs.size(3);
		OctreeTrivecRasterizer::CUDA::backward(
			...
			grad_trivecs.contiguous().data_ptr<float>(),
	...
	return std::make_tuple(
		grad_trivecs, grad_densities, grad_shs, grad_colors, aux_grad_colors2, aux_contributions
	);
}
