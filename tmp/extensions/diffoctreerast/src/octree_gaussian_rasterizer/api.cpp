...
OctreeGaussianRasterizer::rasterizeOctreeGaussiansCUDA(
	...
	if (P > 0) {
		...
		rendered = OctreeGaussianRasterizer::CUDA::forward(
...
OctreeGaussianRasterizer::rasterizeOctreeGaussiansBackwardCUDA(
	const torch::Tensor& background,
	const torch::Tensor& positions,
	const torch::Tensor& colors,
	const torch::Tensor& opacities,
	...
	torch::Tensor grad_opacities = torch::zeros({P, 1}, positions.options());
	...
	// Call Backward
	if (P > 0) {
		OctreeGaussianRasterizer::CUDA::backward(
	...
	return std::make_tuple(
		grad_shs, grad_colors, grad_opacities, aux_grad_colors2, aux_contributions
	);
}
